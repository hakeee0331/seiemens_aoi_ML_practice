...
### 비교는 비율이 아니라 TN/FN 원본 건수로 (2026-08-24 확정)

두 대상(baseline vs 기법, A vs B 등)을 비교할 때는 Slip Rate/Volume
Reduction 같은 **비율만 보여주지 않는다.** 예측 1(불량 의심)로 남는 TP/FP는
baseline(전량 수동검사) 대비 처리 결과가 동일하므로, 실제 변화는 예측
0(자동 정상처리)으로 옮겨간 **TN(이득: 불필요한 검사 감소)**과 **FN(손해:
원래 걸렸을 불량이 유출)** 두 칸에서만 생긴다. 그래서 비교 표에는 항상
**baseline 대비 ΔTN, ΔFN을 원본 건수로 같이 적는다** (비율/퍼센트만 적고
끝내지 않는다). 필요하면 ΔTN/ΔFN 교환비도 참고로 덧붙이되, 교환비 자체가
결론은 아니다 — 최종 판단은 위 총비용 시나리오로 한다.
...
### 시도 로그 (2026-08-24 확정, lsw 개인 작업 범위)

한 실험 노트북 안에서 여러 변형(불균형 처리 기법, feature 조합, 모델 구조 등)을
비교해보는 경우, 시도마다 별도 stem을 새로 만들지 않는다. 대신 해당 실험
문서(`docs/experiments/<stem>.md`)에 "시도 로그" 표를 추가해 각 시도를 한 행씩
기록한다. (팀 공통 규칙이 아니라 lsw 개인 작업에 한정 — 팀 전체 규칙으로 반영할지는
`CONTRIBUTING.md`를 건드리지 않고 필요 시 팀과 별도로 논의한다.)

| 시도 | 변경 내용 | ΔTN(이득) | ΔFN(손해) | Slip Rate | Volume Reduction | 총비용(1:10 / 1:100) | 채택 |
|---|---|---:|---:|---:|---:|---|---|
...
### 새 기법 채택 기준: baseline이 아니라 "현재 최고 기법" 대비 (2026-08-25 확정)

`0825_lsw_005`까지는 새 기법을 항상 baseline과만 비교했다. 그런데 이 방식 때문에 type1에서
recency weighting이 "baseline보다 낫다"고 착각했다가, 004에서 이미 확정된 undersample(더 저렴)과
뒤늦게 직접 비교해서야 실수를 잡은 적이 있다. **`0825_lsw_007`부터는 새 기법의 채택 여부를 항상
그 유형의 "현재까지 알려진 최고 기법"과 직접 비교해서 판단한다** (baseline과 비교해서 이겼다는
이유만으로 채택하지 않는다). 단, baseline 대비 ΔTN/ΔFN은 원인 설명에 유용하므로(왜 되는지 이해하는
용도) 보조 지표로는 계속 같이 기록한다.

- **채택 기준**: 두 비용비율(1:10, 1:100) 모두에서 현재 최고 기법의 총비용보다 낮아야 한다.
- **보조 기록**: baseline 대비 ΔTN/ΔFN, 총비용도 계속 남긴다(비교는 비율이 아니라 원본 건수로
  — 위 규칙과 동일).
- 유형별 "현재 최고"는 비용비율마다 다를 수 있으므로(예: type3은 1:10=adasyn, 1:100=smote였음)
  항상 두 비용비율을 따로 비교한다.

### Feature selection은 보류 (2026-08-25 확정)

원래 다음 단계로 "mapping.json 기반 feature selection"(검사유형별 무효 컬럼
제거)을 계획했으나, dev에 이미 합쳐진 dongjin의 실험 두 건이 이 방향을 사실상
결론 내버렸다.

- `0824_dongjin_006/007/008` (sentinel masking + MoE): type별 100% 0.0인
  컬럼을 NaN 마스킹한 뒤 5분리(MoE) 구조로 학습 → **마스킹 없는 MoE와 소수점까지
  100% 동일한 결과**. 이유: 5분리하면 무효 컬럼은 그 type 안에서 이미 상수가
  되고 XGBoost는 상수 피처로 분기를 못 만드니 자동 무시된다. 우리 003/004의
  `get_non_constant_columns`가 이미 같은 효과를 내고 있었다는 뜻이기도 하다.
- `0824_dongjin_020_dim_reduction`: RuleFit+SHAP 기준 핵심 21개만 남기고 나머지
  54개 제거(importance 기반 축소) → PR-AUC 0.456→0.326으로 폭락, FP 430건 증가.
  "약한 피처들의 조합이 엣지케이스 보정에 쓰인다"는 결론.

두 갈래(도메인 기반 마스킹 / importance 기반 축소) 모두 이미 시도되어 각각
no-op과 손해로 확인됐으므로, **feature selection은 재검증 없이 보류**하고 다음
우선순위(drift 대응 / 라벨 강건 학습)로 넘어간다. 위 "Compact 모델 경량화"
항목의 feature selection도 같은 이유로 별도 기대효과 없이 참고만 한다.
...
- [x] 2-1. 132일을 주 단위(또는 적절한 크기) 윈도우로 분할 — kimjaehak `002_temporal_drift` +
      lsw `005`/`006`에서 주/구간 단위로 다룸
- [x] 2-2. 윈도우 간 feature 분포 변화량 정량화 (PSI 또는 KS-test) → drift가
      실제로 언제·어떤 피처에서 발생하는지 표/그래프로 제시 — kimjaehak `002_temporal_drift`
      (PSI), lsw notes.md의 라벨 판정기준 변화(concept drift) 분석으로 보강
- [x] 2-3. 고정 모델(Phase 0 baseline)을 시간에 따라 그대로 적용했을 때 성능이
      실제로 열화되는지 rolling evaluation으로 확인 — `0825_lsw_006`: 매번 최신 40%로 재학습해도
      type0/1/3은 마지막 구간에서 Volume Reduction 급락(재학습으로도 못 막는 열화 확인)
- [x] 2-4. drift 대응 전략 1개 이상 구현: (b) 최근 데이터에 더 큰 가중치 부여(`0825_lsw_005`
      recency weighting), (a) sliding-window 재학습 시뮬레이션(`0825_lsw_006`). (c) ADWIN/DDM류
      온라인 감지기는 시도 안 함 — 정적 데이터셋이라 온라인 감지 시뮬레이션 근거가 약하다고 판단
      (`0825_lsw_005` 결론 참고)
- [x] 2-5. 대응 적용 전/후 성능 안정성(시간에 따른 지표 변동폭) 비교 — `0825_lsw_005`/`006` 문서
      참고. **결론: 두 대응 모두 004의 검사유형별 최적 조합 순위를 못 뒤집었고, 재학습을 계속해도
      drift를 완전히 상쇄하지 못함을 확인** — Phase 2는 이 결론으로 완료 처리.
