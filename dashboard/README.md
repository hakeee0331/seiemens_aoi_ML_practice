...
CSV Test 구간을 0.3초마다 한 행씩 읽고 그때마다 모델 추론을 수행한다. 모델 정상
건은 화면에 0.3초 동안 표시한 뒤 자동 기록하고, 모델 불량 건에서는 스트림을 멈춰
작업자의 정상/실제 불량 판정을 받는 Streamlit 데모다. 데이터베이스를 사용하지
않으므로 앱 세션이 초기화되면 판정 기록도 사라진다.
...
큐를 만들지 않는다. Streamlit fragment가 0.3초마다 다음 행 하나만 추론한다. 공통
threshold `0.0007097449` 미만이면 현재 화면에 `MODEL NORMAL`로 표시하고 다음
tick에 `모델판정 정상`으로 기록한다. threshold 이상이면 자동 tick의 데이터 진행을
멈추고 해당 행을 작업자에게 노출한다.
...
우측 `INSPECTION LOG` 제목 옆의 `FULL LIST` 버튼으로 전체 검사 내역 화면을 연다.
목록 화면에서도 같은 0.3초 스트림 처리기가 동작하므로 모델 정상 건은 계속
누적된다. 모델 불량 건을 만나면 `MANUAL REVIEW` 상태로 멈추며 `LIVE DASHBOARD`
버튼으로 돌아가 작업자 판정을 수행한다.

목록은 현재 입력 CSV와 동일한 컬럼명·순서를 사용하고 내부 `_stream_order`나 모델
확률 같은 운영 메타데이터는 포함하지 않는다. 입력 Test CSV의 기존 정답은 사용하지
않으며 처리 결과로 `class`를 새로 구성한다.

- 모델 자동 정상: `class = 0`
- 작업자 정상 판정: `class = 0`
- 작업자 실제 불량 판정: `class = 1`

모델 자동 정상은 작업자 검증 라벨이 아닌 pseudo-label이다. 판정 출처는 재학습 CSV
스키마와 분리해 세션 히스토리 메타데이터에 유지한다.

## 현재 데모 범위

- 불량 확률은 Inspection Type별 4개 checkpoint XGBoost 모델의 확률 평균으로
  계산한다.
- Test 행을 0.3초마다 하나씩 단건 추론하며 미래 행을 미리 판정하지 않는다.
- 모델 정상 행은 현재 검사 영역에 0.3초 동안 표시한 뒤 `모델판정 정상`으로 히스토리에
  기록하고 다음 행으로 진행한다.
- 모델 불량 행에서는 `MANUAL REVIEW` 상태로 스트림 진행을 멈춘다. 작업자 판정
  버튼을 누르면 다음 행 하나를 즉시 추론해 화면을 교체하고, 이후 자동 스트림은
  다시 0.3초 간격으로 진행한다.
- 헤더에는 `RUNNING`, `MANUAL REVIEW`, `FINISHED` 상태와 Test 스트림 위치 및 현재까지
  발견한 수동 검사 순번을 표시한다. 미래 수동 검사 총건수는 계산하지 않는다.
- 전체 검사 내역 화면은 원본 CSV 스키마로 처리 행을 시간순 적층하고 모든 행의
  `class`를 모델 또는 작업자 판정으로 채운다.
...
- 히스토리에는 `모델판정 정상`, `작업자판정 정상`, `작업자판정 실제 불량`을 시간순으로
  함께 표시한다.
- 스트림 위치와 상태, 누적 검사 내역, 모델 판정, 작업자 판정 및 직전 검사 결과는
  `st.session_state`에만 저장한다.
