...
- 우측 `INSPECTION LOG` 제목 옆 `FULL LIST` 버튼으로 별도 전체 화면을 연다.
- 목록 화면에서도 공유 0.3초 스트림 처리기가 계속 동작한다.
- 모델 불량 건에서는 목록 적층이 멈추고 `MANUAL REVIEW` 판정 대기를 표시한다.
- `LIVE DASHBOARD` 버튼으로 돌아가 작업자 판정을 수행할 수 있다.
- 처리된 Test 행은 입력 CSV와 같은 컬럼명·순서로 시간순 적층한다.
- 입력 CSV의 원래 `class`는 복사하지 않고 모델 자동 정상과 작업자 정상은 `0`,
  작업자 실제 불량은 `1`로 새로 기록한다.
- 모델 확률, 판정 출처와 `_stream_order`는 세션 메타데이터에 유지하고 재학습용 표에는
  추가하지 않는다.
...
- 전체 목록: 원본 CSV 스키마의 누적 검사 내역과 LIVE 화면 복귀 버튼
...
- 큐 중간 재시작을 위한 체크포인트
- 누적 검사 내역의 검색·필터 기능과 영구 추론 감사 로그
- 누적 검사 내역의 파일 저장·다운로드 및 영구 재학습 데이터셋 관리
- 작업자 인증 및 권한 관리
...
- 전체 목록 화면에서도 0.3초 스트림이 계속 적층되는 것 확인
- 원본 CSV 컬럼 순서 보존과 모델·작업자 판정 기반 `class=0/1` 생성 확인
