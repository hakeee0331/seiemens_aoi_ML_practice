...
| `0825_kimjaehak_017_dongjin_026_threshold_fix` | 완료 | kimjaehak | dongjin_026의 고정 0.5 SHAP와 peace_005 Validation 공통 Threshold SHAP 비교 | XGBoost Fold ensemble SHAP 진단 | 기준 confusion matrix 재현; FP 분석 대상 159→41,118건, Threshold별 Top 5 피처가 Type 1/2/3에서 달라짐 |
...
| `0825_lsw_012_psi_triggered_retrain` | 완료 | lsw | covariate shift(PSI)가 지배적인 type1/4에 한정해 PSI 트리거 기반 재학습이 도움이 되는지 검증 | Type-conditioned XGBoost (walk-forward, type1/4) | 가설 기각 — type1은 재학습이 Slip Rate를 48~79%로 무너뜨림, type4는 표본 부족으로 판단 불가. drift 종류와 무관하게 "확정 데이터로 고른 임계값이 다음 구간에서 안전하다"는 가정 자체가 자주 깨진다는 011과 같은 결론으로 수렴 |
